...
def fetchSkillDetailsFromTable():
    ...
    sql_select_Query                = "select `skill`, `synonyms`,`primary_level_skillset`, `secondary_level_skillset` from "+str(sql_table)+" where status=1"+str(";")
...
def buildGraph(records):
    ...
    for row in records:
        ...
        if(row[2] != None):
            ...
            primary_level_skill_list  = primary_level_skillset_string.split(",")
        ...
        if (row[3] != None):
            ...
            secondary_level_skill_list = secondary_level_skillset_string.split(",")
    ...
    graphname                               = "skillGraph_"+str(current_time)+".gpickle"
    graph_storage_path                      = graph_storage_path +"/"+graphname
